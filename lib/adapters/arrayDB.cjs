...
class CollectionDB {
    ...
    static generateNoActorError = (status, actorID) => new returnData.ReturnData({
        status,
        messages: [`This actor (${actorID}) doesn't exists`],
    });
    static generateOnlyAdminError = (status) => new returnData.ReturnData({
        status,
        messages: ['Only SuperAdmin can read all data'],
    });
    ...
    countAll = async (actorID) => {
        ...
        return new returnData.ReturnData({ status: 225, payload });
    };
