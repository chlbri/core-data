import { recompose, decompose } from '@bemedev/decompose';
import { ReturnData } from '@bemedev/return-data';
import { produce, castDraft } from 'immer';
import { nanoid } from 'nanoid';
import { merge } from 'ts-deepmerge';
import '../schemas/index.js';
import { inStreamSearchAdapter, zodDecomposeKeys, withProjection, withProjection2 } from './arrayDB.functions.js';
import { timestampsSchema } from '../schemas/objects.js';
...
class CollectionDB {
    ...
    generateServerError = (status, ...messages) => {
    ...
    static buildCreate(actorID, data, _id = nanoid()) {
    ...
    static get timestampsPermissionsCreator() {
        const keys = Object.keys(timestampsSchema.shape);
    ...
    _createData = (actorID, data, _id = nanoid()) => {
        const input = CollectionDB.buildCreate(actorID, data, _id);
        this.pushData(input);
        return input;
    };
    generateCreate = (actorID, data, _id = nanoid()) => {
    ...
    createMany = async ({ actorID, data: _datas, options, }) => {
        ...
        if (!canCreate) {
            return this.generateServerError(510, 'This actor cannot create elements');
        }
        // #endregion
    ...
    createOne = async ({ data, actorID }) => {
        ...
        const canCreate = this.canCreate(actorID);
        if (!canCreate) {
            return this.generateServerError(511, 'This actor cannot create elements');
        }
        // #endregion
    ...
    upsertOne = async ({ actorID, id: _id, data }) => {
        ...
        const canCreate = this.canCreate(actorID);
        if (!canCreate) {
            return this.generateServerError(512, 'This actor cannot create elements');
        }
        // #endregion
    ...
    upsertMany = async ({ actorID, upserts, options }) => {
        const canCreate = this.canCreate(actorID);
        if (!canCreate) {
            return this.generateServerError(513, 'This actor cannot create elements');
        }
    ...
    _reduceByPermissions = ({ actor, filters, ids, options, }) => {
         ...
         */
        const flattenPermissions = rawPermissions.map(decompose);
        const payload = [];
        flattenPermissions.forEach(({ _id, ...perm }, index) => {
            ...
            if (check1) {
                payload.push(flattenReadsWithProjection[index]);
            }
            // #endregion
            else {
    ...
    countAll = async (actorID) => {
        const actor = this._canRead(actorID);
        if (actor !== true)
            return this.generateServerError(525, 'Only superadmin can count all');
    ...
    count = async ({ actorID, filters, options }) => {
        ...
        const payload = this._collection.filter(inStreamSearchAdapter(filters)).length;
        if (payload <= 0) {
    ...
    updateAll = async ({ actorID, data, options }) => {
        ...
        const helper = [];
        const db = merge(helper, this._collection);
        const limit = options?.limit;
        const inputs = db
            .slice(0, limit)
            .map(_data => merge(_data, data));
