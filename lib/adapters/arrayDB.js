...
class CollectionDB {
    ...
    reducePermissionsFirstStep(ids, filters, options) {
        ...
        const rawReadsWithProjection = slicedReads.map(data => withProjection(data, ...projection));
        return {
            slicedReads,
            projection,
            isLimited,
    ...
    generateNoActor = (status, actorID) => new ReturnData({
        status,
        messages: [
    ...
    updateManyByIdsWithOne = async ({ ids, filters, update, options, actorID, }) => {
        ...
        const { payload: rawPayload, isLimited,
        // isRestricted,
