...
import { inStreamSearchAdapter, withProjection, withProjection2, countOccurences, zodDecomposeKeys, intersectObjects } from './arrayDB.functions.js';
...
class CollectionDB {
    ...
    static generateNoActorError = (status, actorID) => new ReturnData({
        status,
        messages: [`This actor (${actorID}) doesn't exists`],
    });
    static generateOnlyAdminError = (status) => new ReturnData({
        status,
        messages: ['Only SuperAdmin can read all data'],
    });
    generateActorCannotPerform = ({ status, actorID, action = 'read', }) => new ReturnData({
        status,
        messages: [
            `This actor ${actorID ? `${actorID} :` : ':'} cannot ${action} data`,
        ],
    });
    static generateRedirect(status, ...messages) {
        return new ReturnData({
            status,
            messages,
        });
    }
    static generateRedirectEmpty(status) {
        return CollectionDB.generateRedirect(status, 'Query return Empty');
    }
    static generateNotFound(status) {
    ...
    __update = (...updates) => {
        ...
        const __db = produce(this._collection, draft => {
            updates.forEach(({ _id, ...update }) => {
                ...
                if (fIndex !== -1) {
                    draft[fIndex] = castDraft(merge(draft[fIndex], update));
                    payload.push(_id);
                }
                else {
                    idsNotFound++;
